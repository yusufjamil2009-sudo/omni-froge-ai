...
export async function readProviderCredentials(providerId: string): Promise<Record<string, string>> {\n  return readCredentials(providerId);\n}\n\nexport async function writeProviderUsage(providerId: string, error: boolean, rateLimited: boolean): Promise<void> {\n  const client = await db();\n  const row = await readRow(providerId);\n  const usage = ((row?.["usage"] as ProviderUsage | null) ?? {}) as ProviderUsage;\n  const now = new Date().toISOString();\n  const { error: dbError } = await client.from(TABLE).update({\n    usage: json({ ...usage, requests: (usage.requests ?? 0) + 1, errors: (usage.errors ?? 0) + (error ? 1 : 0), rateLimits: (usage.rateLimits ?? 0) + (rateLimited ? 1 : 0), lastUsedAt: now }),\n    last_tested_at: now,\n    last_error: null,\n    connection_status: "WORKING",\n  }).eq("provider_id", providerId);\n  if (dbError) throw new Error(dbError.message);\n}\n\nexport async function writeProviderRuntimeFailure(providerId: string, errorClass: ProviderFailure["errorClass"], reason: string, retryAfterSeconds: number | null): Promise<void> {\n  const client = await db();\n  const row = await readRow(providerId);\n  const usage = ((row?.["usage"] as ProviderUsage | null) ?? {}) as ProviderUsage;\n  const now = new Date().toISOString();\n  const failure: ProviderFailure = { errorClass, reason: reason.slice(0, 240), at: now, retryAfterSeconds };\n  const status = statusForError(errorClass);\n  const { error: dbError } = await client.from(TABLE).update({\n    connection_status: status, last_tested_at: now, last_error: json(failure),\n    usage: json({ ...usage, requests: (usage.requests ?? 0) + 1, errors: (usage.errors ?? 0) + 1, rateLimits: (usage.rateLimits ?? 0) + (errorClass === "RATE_LIMITED" ? 1 : 0), lastUsedAt: now }),\n  }).eq("provider_id", providerId);\n  if (dbError) throw new Error(dbError.message);\n}\n\nexport type SaveResult =
  | { ok: true }
